...
export default {
  ...
  methods: {
    //获得音乐json文件
    getMusicData() {
		// http://softeyes-xl.github.io/music/SFVer/static/music.json
      axios
        .get("../../static/music.json")
        .then(
          response => {
	...
	//   http://softeyes-xl.github.io/music/SFVer/static/lrc/
    ...
    playMusic() {
      ...
      if (that.$refs.audio !== null) {
        if (that.$refs.listul2.getAttribute("myplay") == "false") {
          that.$refs.listul.children[that.playNum].setAttribute(
            "class",
            "playing"
          );
          if (that.ifPlay) {
    ...
    mainCanvas: function() {
      ...
      function chose(particleName) {
        particleName.addEventListener(
          "click",
          function(e) {
            this.style.backgroundColor = "orange";
            (particleName == ball ? rect : ball).style.backgroundColor =
              "rgba(0, 0, 0, 0)";
  ...
  mounted() {
    ...
    let imgspan = document
      .getElementsByClassName("choose-img")[0]
      .getElementsByClassName("imgurl");
    ...
    audio.addEventListener("timeupdate", function() {
      ...
      if (audio.ended) {
        if (that.$refs.listul2.getAttribute("myplay") == "false") {
          ...
          for (var i = 0; i < that.$refs.listul2.children.length; i++) {
            if (
              that.$refs.listul2.children[i].getAttribute("class") == "addmy"
            ) {
            } else {
              that.$refs.listul2.children[i].setAttribute("class", "");
            }
