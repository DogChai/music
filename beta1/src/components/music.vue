<template>
  <div id="music-wrap">
    <div class="music">
      <audio id="audio" ref="audio" crossorigin="anonymous" data-num="0"></audio>
      <input type="file" accept="audio/mp3" id="input-file" class="hidden file">
      <div class="show-title show-title-hide" id="show-title">顺序播放</div>
      <div class="music-wrap">
        ...
        <div class="music-show">
          <div class="music-name">
            <div class="music-show-name" ref="showname"></div>
        ...
        <div class="music-vol">
          ...
          <div class="vol-bar" v-on:click.stop="volClick">
            <div class="vol-plan"></div>
            <div class="vol-bar-btn" v-on:mousedown.stop="volDrag"></div>
          </div>
        </div>
    ...
    <ul class="list-header-ul" ref="headerul">
      <li class="choose" v-on:click="showList1()">线上音乐</li>
      <li v-on:click="showList2()">我的音乐</li>
    </ul>
    <div class="music-play-list" ref="playlist" ifshow="0" >
      <div class="play-list-header" v-on:click.stop="headerClick()">Play List</div>

      <!-- <div class="list-bar-wrap">
        <div class="list-bar" id="list-bar" v-on:mousedown='listBar'></div>
      </div>-->
      <div class="play-list-cut" ref="listul" style="opacity: 1; z-index: 999">
        <v-scroll>
          <ul class="play-list-ul list-ul" ref="listChild1">
            <li
              :key="index"
              v-for="(item,index) in musicData"
              v-on:click="listClick()"
              :num="index"
              :data-id='item.id'
              :musicName="item.name"
            >
      ...
      <div class='play-list-cut' ref="listul2" myplay="false" style="opacity: 0; z-index: -999">

        <v-scroll>
          <ul class="list-ul list-ul2" ref="listChild2">
            <li
              :key="index"
              ...
              :musicName="item.name"
              :datasrc="item.url"
              v-on:click.stop="listClick2()"
            >
              <span>{{index + 1}}.</span>
              {{item.name}} - {{item.singer}}
              <div class="ul2-close" v-on:click.stop="removeLi()">×</div>
            </li>
            <li class="addmy">
              <!-- 点击添加 -->
              <input
    ...
    <div class="music-top" ref="musictop">
      ...
      <div class="choose-img2 choose-img">
        <ul>
          <li class="imgurl current" imgurl="http://data.wuyayu.com/201901.png"></li>
          <li class="imgurl" imgurl="http://data.wuyayu.com/201902.png"></li>
          <li class="imgurl" imgurl="http://data.wuyayu.com/201903.png"></li>
          <li class="imgurl" imgurl="http://data.wuyayu.com/201905.png"></li>
          <li class="imgurl" imgurl="http://data.wuyayu.com/201904.png"></li>
          <li class="imgurl" imgurl="http://data.wuyayu.com/201906.png"></li>
        </ul>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      waveWidth: 0,
      waveHeight: 0,
      // "rgb(255,239,252)", "rgb(217,82,80)", "rgb(150,120,110)"
      c1: "rgb(255,239,252)",
      c2: "rgb(217,82,80)",
      c3: "rgb(150,120,110)",
      cutHeight: 0,
  ...
  methods: {
    // 请求网易云歌曲
    getMusicUrl(id) {
      axios.get('http://node.wuyayu.com:3000/v1/music/url?id='+id+'&br=128000').then(response => {
    ...
    dragMusic() {
      ...
      document.addEventListener("drop", function(e) {
        ...
        for (var i = 0; i < thisFiles.length; i++) {
          ...
          if (ext == "mp3" || ext == "flac") {
            let dragName = thisFiles[i].name.substring(
              0,
              thisFiles[i].name.lastIndexOf(".")
            );
    ...
    getInput() {
      ...
      if (ext == "mp3" || ext == "flac") {
        let dragName = thisFiles[0].name.substring(
          0,
          thisFiles[0].name.lastIndexOf(".")
        );
    ...
    getLrcFile(n) {
      ...
      // http://music.163.comhttp://node.wuyayu.com:3000/v1/song/lyric?os=pc&id=93920&lv=-1&kv=-1&tv=-1
      ...
      if (this.$refs.listul2.getAttribute("myplay") == "false") {
        ...
        // lyric?id=1336856864
        // ../../static/lrc/
        axios.get("http://node.wuyayu.com:3000/v1/lyric?id=" + n).then(
          response => {
            // if (
            //   response.data == "纯音乐,请欣赏" ||
            //   response.data == "后摇,请欣赏" ||
            //   response.data == "暂无歌词"
            // ) {
            ...
            if(response.data.lrc) {
              this.lrcData = response.data.lrc.lyric;
    ...
    playMusic() {
      ...
      for (var i = 0; i < this.musicData.length; i++) {
        document.getElementsByClassName('list-ul')[0].children[i].setAttribute("class", "");
      }

      if(this.ifMy == 0) {
        document.getElementsByClassName('list-ul')[0].children[this.playNum].setAttribute("class", "playing");
        if (that.ifPlay) {
      ...
      //   if (document.getElementsByClassName('list-ul2')[0].getAttribute("myplay") == "false") {
      //     document.getElementsByClassName('list-ul')[0].children[that.playNum].setAttribute(
      //       "class",
      //       "playing"
      //     );
      //     if (that.ifPlay) {
    ...
    volDrag() {
      ...
      document.onmousemove = function(e) {
        ...
        that.volLeft =
          e.clientX -
          volBar.offsetLeft -
          musicWrap.offsetLeft -
          volBarBtn.offsetWidth / 2;
    ...
    planClick(e) {
      ...
      let musicWrap = document.getElementsByClassName("music-wrap")[0];
      this.musicLeft =
        e.clientX - musicWrap.offsetLeft - musicBar.offsetLeft - 6;
    ...
    showList() {
      if (this.listShow == 0) {
        ...
        this.$refs.musicwave.style.height =
          window.innerHeight - 85 - 200 + "px";
        this.dragScroll();
        this.listShow = 1;
        ...
        this.$refs.headerul.style.top = '4px';
      } else {
        ...
        this.$refs.musiclrc.style.height =
          this.$refs.musiclrc.offsetHeight + 50 + "px";
        this.$refs.musicwave.style.height =
          this.$refs.musicwave.offsetHeight + 50 + "px";
        this.dragScroll();
        this.listShow = 0;
        this.$refs.playlist.setAttribute("ifshow", 0);
        this.waveWidth = this.$refs.musicwave.offsetWidth;
        this.waveHeight = this.$refs.musicwave.offsetHeight;
        this.$refs.headerul.style.top = '-120px';
      }
    },
    //加载完成获得第一首音乐的信息
    firstGet() {
      let url = ''
      axios.get('http://node.wuyayu.com:3000/v1/music/url?id='+this.musicData[0].id+'&br=128000').then(response => {
        url =  response.data.data[0].url;

        this.gMusicUrl = url;
        this.$refs.audio.src = url;
        this.$refs.showname.innerHTML =
          this.musicData[0].name + " - " + this.musicData[0].singer;
        let that = this;
        this.$refs.audio.addEventListener("canplaythrough", function() {
          that.timeActive();
        });
        this.getLrcFile(this.musicData[0].id);
      ...
      if (this.$refs.listul2.getAttribute("myplay") == "false") {
        ...
        if (this.$refs.lrcul.getAttribute("move") == "true") {
          ...
          for (var i = 0; i < lrcli.length; i++) {
            var thisLiHeight = 0;
            if (
              lrcli[i].getAttribute("time") == this.curTime &&
              lrcli[i].getAttribute("ifchoose") == "false"
            ) {
              ...
              if (lrcLine === 2) {
                ...
                if (i - 2 < 0 || i - 1 < 0) {
                  ...
                  // i2 <= 0 ? 1 : i2;
                  thisLiHeight =
                    lrcli[i].offsetHeight + lrcli[i - 1].offsetHeight;
                }
              ...
              if (lrcLine === 3) {
                ...
                thisLiHeight =
                  lrcli[i].offsetHeight +
                  lrcli[i - 1].offsetHeight +
                  lrcli[i - 2].offsetHeight;
                thisLiHeight = thisLiHeight / 2;
              }
              if (lrcLine === 4) {
                lrcli[i].setAttribute("class", "playing");
                ...
                thisLiHeight =
                  ...
                  lrcli[i - 1].offsetHeight +
                  lrcli[i - 2].offsetHeight +
                  lrcli[i - 3].offsetHeight / 2;
              }
              let lrcTop = parseInt(
                this.$refs.lrcul.style.transform.substring(11)
              );
        ...
        if (lrcLi.length == 1) {
          ...
          if (lisnum == 0) {
            ...
            if (lisnum < 3) {
              if (lisnum == 1) {
                if (
                  lrcLi[0].getAttribute("time") == lrcLi[1].getAttribute("time")
                ) {
              ...
              if (
                lrcLi[lisnum - 1].getAttribute("time") ==
                lrcLi[lisnum - 2].getAttribute("time")
              ) {
                ...
                lrcLi[lisnum - 1].getAttribute("time") ==
                lrcLi[lisnum].getAttribute("time")
              ) {
                lrcLi[lisnum - 1].setAttribute("class", "playing");
    ...
    musicEndsPlay() {
      if (this.$refs.listul2.getAttribute("myplay") == "false") {
        ...
        axios.get('http://node.wuyayu.com:3000/v1/music/url?id='+this.musicData[this.playNum].id+'&br=128000').then(response => {
          ...
          this.$refs.showname.innerHTML =
            this.musicData[this.playNum].name +
            " - " +
            this.musicData[this.playNum].singer;
          this.getMusicTime();
          this.ifPlay = true;
          this.playMusic();
          this.getLrcFile(this.musicData[this.playNum].id);
        ...
        this.$refs.showname.innerHTML =
          this.dragData[this.dragNum].name +
          " - " +
          this.dragData[this.dragNum].singer;
        this.getMusicTime();
        this.ifPlay = true;
        this.playMusic();
      }
    ...
    showList1() {
      ...
      document.getElementsByClassName('play-list-cut')[1].style.opacity = 0;
      document.getElementsByClassName('play-list-cut')[1].style.zIndex = -999;
      document.getElementsByClassName('play-list-cut')[0].style.opacity = 1;
      document.getElementsByClassName('play-list-cut')[0].style.zIndex = 999;
      document.getElementsByClassName('play-list-cut')[1].setAttribute("myplay", "false");
      this.listNum();
    },
    //切换我的音乐列表
    showList2() {
      ...
      // document.getElementsByClassName('play-list-cut')[1].style.left = "0";
      // document.getElementsByClassName('play-list-cut')[0].style.left = "-230px";
      document.getElementsByClassName('play-list-cut')[1].style.opacity = 1;
      document.getElementsByClassName('play-list-cut')[1].style.zIndex = 999;
      document.getElementsByClassName('play-list-cut')[0].style.opacity = 0;
      document.getElementsByClassName('play-list-cut')[0].style.zIndex = -999;
      document.getElementsByClassName('play-list-cut')[1].setAttribute("myplay", "true");
      this.listNum();
    },
    //列表点击
    listClick() {
      let this_ = event.currentTarget;
      let id = this_.getAttribute("data-id");
      ...
      this.$refs.listChild2.setAttribute("myplay", "false");
      for (var i = 0; i < this.$refs.listChild2.children.length; i++) {
        if (this.$refs.listChild2.children[i].getAttribute("class") == "addmy") {
          //null
        } else {
          this.$refs.listChild2.children[i].setAttribute("class", "");
        }
      }
      ...
      axios.get('http://node.wuyayu.com:3000/v1/music/url?id='+id+'&br=128000').then(response => {
        ...
        if (audioUrls == this.audioUrl) {
          ...
          for (var i = 0; i < this.musicData.length; i++) {
            this.$refs.listChild1.children[i].setAttribute("class", "");
          }
    ...
    listClick2() {
      ...
      if (audioUrls == this.audioUrl) {
        ...
        this.ifPlay = true;
        this.$refs.showname.innerHTML =
          this.dragData[thisNum].name + " - " + this.dragData[thisNum].singer;
        this_.setAttribute("class", "playing");
        this.playMusic();
      }
      this.getLrcFile(this.dragData[this.dragNum].id);
    },
    //切换列表音乐数量
    listNum() {
  ...
  mounted() {
    ...
    // jq('.blur').attr('data-attr','http://data.wuyayu.com/201901.png');
    ...
    for (var i = 0; i < imgspan.length; i++) {
      ...
      imgspan[i].addEventListener("click", function() {
        ...
        if (this.index === 0) {
          changeColor("rgb(24,24,24)", "rgb(255,164,133)", "rgb(109,146,189)");
          ...
          changeColor("rgb(94,20,28)", "rgb(244,70,72)", "rgb(209,147,110)");
        } else if (this.index === 3) {
          changeColor("rgb(44,24,13)", "rgb(240,106,101)", "rgb(117,38,19)");
        } else if (this.index === 4) {
          changeColor("rgb(244,200,141)", "rgb(240,159,120)", "rgb(191,194,202)");
    ...
    function onloadsize() {
      ...
      that.$refs.musiclrc.style.height =
        that.$refs.musiclrc.offsetHeight + 50 + "px";
      that.$refs.musicwave.style.height = window.innerHeight - 85 - 200 + "px";
      that.$refs.musicwave.style.height =
        that.$refs.musicwave.offsetHeight + 50 + "px";
    ...
    audio.addEventListener("timeupdate", function() {
      if (audio.ended) {
        if (that.$refs.listul2.getAttribute("myplay") == "false") {
          ...
          for (var i = 0; i < that.$refs.listChild2.children.length; i++) {
            if (
              that.$refs.listChild2.children[i].getAttribute("class") == "addmy"
            ) {
            } else {
              that.$refs.listChild2.children[i].setAttribute("class", "");
            }
